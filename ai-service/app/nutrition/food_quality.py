...
def food_quality_feedback(dishes: list[str], notes: str | None = None) -> dict[str, Any]:
    ...
    if oily:
        suggestions.append(
            "This plate looks oil-forward — if you want a lighter swap, try air-fried or sautéed versions "
            "and keep portions editable."
        )
    if notes and "late" in notes.lower():
        suggestions.append("Late eating noted — consider an earlier window when schedule allows (advisory).")
    if not suggestions:
    ...
    return {
        ...
        "improvedRecipeHint": "Request a lighter recipe variant in chat — output stays user-editable.",
        "editable": True,
    }
