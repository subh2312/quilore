...
| Goal-based calorie, macro, and micronutrient targets | Calculate daily calorie, macro, and core micronutrient targets based on goal, weight, height, and related profile inputs; let users track progress against those targets | P0 |
| Micronutrient tracking & shortfall flags | Track a core micronutrient set (fiber, sodium, potassium, iron, calcium, vitamin D, vitamin B12, vitamin C) from IFCT 2017 data across scanned and manual meals, flagging likely shortfalls (e.g., low iron, low B12) | P1 |
...
- Comprehensive micronutrient tracking across the full IFCT 2017 nutrient panel — v1 scopes to a core set of eight micronutrients (fiber, sodium, potassium, iron, calcium, vitamin D, vitamin B12, vitamin C), with the architecture extensible to the full panel later.
...
- Micronutrient goal adoption and logging rate — percentage of active users who view or log against micronutrient targets.
- Shortfall-flag precision — accuracy of low-iron/low-B12/etc. flags against ground-truth intake, validated against user-reported or coach-reviewed logs.
...
| meal_item | id, meal_entry_id, food_name, source_confidence, portion_value, portion_unit, grams, calories, protein_g, carbs_g, fat_g, fiber_g, sodium_mg, potassium_mg, iron_mg, calcium_mg, vitamin_d_mcg, vitamin_b12_mcg, vitamin_c_mg | Stores food-line items and computed macros plus the core micronutrient set |
| nutrition_goal | id, user_id, goal_type, weight_kg, height_cm, age, sex, activity_level, target_calories, target_protein_g, target_carbs_g, target_fat_g, target_fiber_g, target_sodium_mg, target_potassium_mg, target_iron_mg, target_calcium_mg, target_vitamin_d_mcg, target_vitamin_b12_mcg, target_vitamin_c_mg, effective_from | Stores daily calorie, macro, and micronutrient targets derived from user profile and selected goal |
| nutrition_progress_snapshot | id, user_id, date, consumed_calories, consumed_protein_g, consumed_carbs_g, consumed_fat_g, consumed_fiber_g, consumed_sodium_mg, consumed_potassium_mg, consumed_iron_mg, consumed_calcium_mg, consumed_vitamin_d_mcg, consumed_vitamin_b12_mcg, consumed_vitamin_c_mg, target_calories, target_protein_g, target_carbs_g, target_fat_g, shortfall_flags (JSONB) | Stores daily rollups for progress views, analytics, and micronutrient shortfall detection |
...
7. Nutrient values — including calories, macros, and the core micronutrient set (fiber, sodium, potassium, iron, calcium, vitamin D, vitamin B12, vitamin C) — are queried from IFCT 2017; results are shown as a range with a manual gram-adjustment slider, since single-image volume estimation is an inherently ill-posed scale problem.
...
5. The backend calculates calories, protein, carbs, fat, and the core micronutrient set automatically for each line item and aggregates meal totals.
6. Users can edit food matches, serving units, and grams before final save.

## 2.10 Goal-Based Macro & Micronutrient Target Engine
...
4. Micronutrient targets (fiber, sodium, potassium, iron, calcium, vitamin D, vitamin B12, vitamin C) are derived independently from ICMR-NIN RDA tables, based on age, sex, and activity level, rather than from the calorie surplus/deficit logic.
5. Targets are versioned so historic meal logs remain tied to the goal state active on that date.
6. Daily progress services aggregate consumed vs. target calories, macros, and micronutrients for charts, streaks, shortfall flags, and coaching prompts.

***
# 3. System Architecture

## 3.1 High-Level Topology
The architecture separates four layers: client (React Native), backend (Spring Boot + FastAPI), data (PostgreSQL/pgvector + MinIO), and external services (cloud AI providers, wger, IFCT 2017, ICMR-NIN RDA guidelines). The mobile client talks to Spring Boot directly for auth, logs, sync, nutrition targets, manual meal entry persistence, and progress tracking, and to FastAPI for anything requiring AI processing; FastAPI in turn hands validated, structured results back to Spring Boot for persistence, ensuring AI never writes directly to the source-of-truth database without business-rule validation.
...
Scanned food photos are processed through segmentation, classification, and depth estimation on cloud APIs, presented to the user for confirmation, converted to a nutrient estimate (macros and core micronutrients) against IFCT 2017, and enriched with LLM-generated quality feedback before being persisted as a log entry. Manual meal entries bypass the vision stack and go through Spring Boot nutrition resolution services to calculate calories, macros, and micronutrients directly from food names and portion sizes.

## 3.4 Goal Tracking Data Flow
The user sets a nutrition goal by entering objective, body metrics, and related profile inputs. Spring Boot computes target calories, macro splits, and core micronutrient targets (via ICMR-NIN RDA tables), stores the effective target set, and updates daily progress snapshots — including shortfall flags — as meals are logged through either scanning or manual entry. The coaching layer can then use these snapshots to answer questions such as whether the user is under protein, over calories, running low on iron or B12, or aligned with the selected goal.
...
| IFCT 2017 | Indian nutrient composition database (528 foods) | Published by NIN/ICMR; used as authoritative nutrient source for macros and micronutrients |
...
| ICMR-NIN RDA Guidelines | Recommended Dietary Allowances by age, sex, and activity level | Used to derive core micronutrient targets independent of calorie surplus/deficit logic |
...
| Macro & micronutrient target progress card | Nutrition home, daily summary, profile goal setup | Shows consumed vs. target calories, protein, carbs, fat, and core micronutrients with remaining balance and visual progress bars |
| Micronutrient shortfall flag | Nutrition home, daily/weekly summary, coach chat | Surfaces a lightweight badge or chat callout (e.g., "Iron trending low this week") when consumed micronutrients fall persistently short of target, worded as an observation rather than a diagnosis |
...
- Feedback from the AI coach (form corrections, nutrition flags, micronutrient shortfalls) is visually distinguished from raw data logs, so users can tell "AI observation" apart from "confirmed fact."
...
- Goal-based calorie, macro, and micronutrient targets are labeled as coaching estimates, with inputs and formulas reviewable by the user so targets do not appear medically prescriptive.
- Micronutrient shortfall flags are worded strictly as observations (e.g., "trending low"), never as medical diagnoses, and always link back to the underlying logged data so users can see why a flag was raised.
